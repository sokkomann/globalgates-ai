...
## AI. - 광고 등록 시, 태그 예측: Okt, CountVectorize, NB
...
분류 모델로 각 태그별 확률을 계산한 뒤, 내림차순 정렬해 상위 3개 클래스를 LabelEncoder로 태그 단어에 매핑합니다.

---

## AI. - 게시글 등록 시, 예상 조회수 분석: TFIDF, 벡터 유사도

> **목적**: 작성 중인 글이 게시되었을 때의 예상 조회수를 미리 보여 작성자가 콘텐츠 강도를 가늠하도록 보조

작성 중인 글과 가장 비슷한 기존 게시글들을 찾아 그 조회수를 가중 평균하여 예상 조회수를 추정합니다. 서버 부팅 시 `피드목록`에서 게시글들을 모두 읽어와 TFIDF로 미리 구축해 두고, 요청이 들어오면 새 글을 동일하게 변환해 코사인 유사도를 계산합니다.
...
## AI. - 지능형 피드 큐레이션 및 답변 생성: Generative Search, Context-Aware 리트리벌
