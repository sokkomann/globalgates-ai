...
GlobalGates는 무역 기반 비즈니스 소셜 마켓 플랫폼으로, 국내 기업과 해외 바이어를 **피드 기반 SNS**로 직접 연결해 정보 격차를 해소합니다. 
...
### 1 — 한국 무역 시장은 25년간 4배 이상 성장

<img src="./gg_data_analysis/gg_1.PNG">
...
## 3. - 광고 등록 시, 어울리는 태그 예측: Okt, CountVectorize, NB

> **목적**: 광고를 등록하는 동안 광고의 제목과 본문을 분석해 어울리는 태그 3개를 즉시 제안

광고를 등록, 신청하는 과정에서 제목과 본문을 분석해 어울리는 태그를 예측하는 모델입니다. 사전에 학습된 분류 모델(`pkl/ad_tag_model.pkl`)과 LabelEncoder(`pkl/ad_tag_encoded.pkl`)를 가져와서, 형태소 분석으로 추출한 명사를 입력 피처로 사용합니다.
...
## 4. - 게시글 등록 시, 예상 조회수 분석: TFIDF, 벡터 유사도

> **목적**: 작성 중인 글이 게시되었을 때의 예상 조회수를 미리 보여 작성자가 콘텐츠 강도를 가늠하도록 보조

작성 중인 글과 가장 비슷한 기존 게시글들을 찾아 그 조회수를 가중 평균하여 예상 조회수를 추정합니다. 서버 부팅 시 `tbl_post`에서 활성 피드를 모두 읽어와 TFIDF로 미리 구축해 두고, 요청이 들어오면 새 글을 동일하게 변환해 코사인 유사도를 계산합니다.
...
## 5. - 지능형 피드 큐레이션 및 답변 생성: Generative Search, Context-Aware 리트리벌
